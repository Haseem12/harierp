...
try {
    ...
    // 1. Get the product ID from the log entry
    $stmt_get_log = $conn->prepare("SELECT productId FROM product_stock_adjustment_logs WHERE id = ? AND adjustmentType = 'PENDING_APPROVAL'");
    $stmt_get_log->bind_param("s", $logId);
    $stmt_get_log->execute();
    $result_log = $stmt_get_log->get_result();

    if ($result_log->num_rows === 0) {
        throw new Exception("Log entry with ID '{$logId}' not found or is not pending approval.");
    }
