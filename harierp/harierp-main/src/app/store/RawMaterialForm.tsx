...
interface RawMaterialFormProps {
  ...
  // onSave prop is no longer needed as form will handle submission directly
}
...
export default function RawMaterialForm({ rawMaterial: existingMaterial }: RawMaterialFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } else if (!imagePreviewUrl && existingMaterial?.imageUrl) { // If preview was cleared and there was an existing image
        imageUrlToSave = ''; // Explicitly set to empty to remove image
    }


    const materialPayload: any = {
      ...formData,
      id: existingMaterial?.id, // Will be undefined for new materials
      ...
      // Ensure dates are in string format for PHP
     ...
     if (!existingMaterial?.id) { 
      delete materialPayload.id; // Remove id for new entries if PHP expects auto-increment or generates it
    }
