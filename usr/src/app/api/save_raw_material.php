...
try {
    ...
    if (
        ...
        throw new Exception("Incomplete or invalid item data provided.");
    }

    $id = isset($data->id) && !empty(trim($data->id)) ? trim($data->id) : 'purch_item_' . uniqid();
    $name = trim($data->name);
    ...
    if (isset($data->id) && !empty(trim($data->id))) {
        ...
        $stmt = $conn->prepare("UPDATE purchase_items SET name=?, description=?, category=?, sku=?, unitOfMeasure=?, litres=?, stock=?, costPrice=?, lowStockThreshold=?, imageUrl=?, supplierId=?, updatedAt=? WHERE id=?");
        ...
        if ($stmt->execute()) {
            http_response_code(200);
            echo json_encode(["success" => true, "message" => "Store item updated successfully.", "id" => $id]);
        } else {
            throw new Exception("Failed to update store item: " . $stmt->error);
        }
    } else {
        // --- Insert New Purchase Item ---
        $createdAt = isset($data->createdAt) ? $data->createdAt : date('Y-m-d H:i:s');
        $stmt = $conn->prepare("INSERT INTO purchase_items (id, name, description, category, sku, unitOfMeasure, litres, stock, costPrice, lowStockThreshold, imageUrl, supplierId, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(["success" => true, "message" => "Store item added successfully.", "id" => $id]);
        } else {
            // Check for duplicate SKU
            if ($conn->errno == 1062) { // 1062 is the MySQL error number for duplicate entry
                throw new Exception("Failed to add item: An item with SKU '{$sku}' already exists.");
            }
