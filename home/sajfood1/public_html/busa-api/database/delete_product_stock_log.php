...
try {
    ...
    // Prevent deletion of system-critical logs from this generic endpoint
    $allowedToDelete = ['MANUAL_CORRECTION_ADD', 'MANUAL_CORRECTION_SUBTRACT', 'INITIAL_STOCK', 'ADDITION', 'PENDING_APPROVAL'];
    if (!in_array($adjustmentType, $allowedToDelete)) {
        throw new Exception("Cannot delete logs of type '{$adjustmentType}'. Only manual adjustments or pending submissions can be deleted here.");
    }
    
    // Reverse the stock adjustment if it wasn't just a pending submission
    ...
    $conn->commit();
    http_response_code(200);
    echo json_encode(["success" => true, "message" => "Stock log entry deleted and any associated stock adjustment has been reversed successfully."]);
