  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEditMode && existingLogEntry) {
      ...
      try {
        ...
        if (result.success) {
            toast({ title: "Stock Log Updated", description: `Log entry updated successfully.` });
      ...
      try {
        ...
        const result = await response.json();
        if (result.success) {
            toast({ title: "Stock Updated", description: `${validItems.length} product(s) stock levels updated.`});
            if (onSaveSuccess) onSaveSuccess(); else router.push('/products/stock-management/log');
        } else { throw new Error(result.message); }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <Select 
            ...
            disabled={isEditMode && (existingLogEntry?.adjustmentType === 'SALE_DEDUCTION' || existingLogEntry?.adjustmentType === 'RETURN_ADDITION' || existingLogEntry?.adjustmentType === 'PENDING_APPROVAL')}
        >
            ...
            <SelectContent>
                {productStockAdjustmentTypes
                  .filter(type => !['SALE_DEDUCTION', 'RETURN_ADDITION', 'PENDING_APPROVAL'].includes(type))
                  .map(type => (
                    <SelectItem key={type} value={type}>{type.replace(/_/g, ' ')}</SelectItem>
                ))}
            </SelectContent>
        </Select>
        {(isEditMode && ['SALE_DEDUCTION', 'RETURN_ADDITION', 'PENDING_APPROVAL'].includes(existingLogEntry?.adjustmentType || '')) && (
            <p className="text-xs text-muted-foreground mt-1">This adjustment type cannot be changed.</p>
        )}
      </div>
      ...
      <div className="flex items-center justify-end gap-2 mt-6">
        ...
        >
          {isLoading ? (isEditMode ? <><RefreshCw className="mr-2 h-4 w-4 animate-spin"/>Updating...</> : <><RefreshCw className="mr-2 h-4 w-4 animate-spin"/>Recording...</>) : (isEditMode ? "Save Changes" : "Record Adjustment(s)")}
        </Button>
      </div>
    </form>
  );
